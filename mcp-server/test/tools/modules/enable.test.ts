/**
 * Tests for the `modules.enable` MCP tool.
 *
 * Flips the persisted `enabled` flag to `true` for a given module id.
 * State is persisted at `<projectRoot>/.forgekit/modules_state.json`.
 * The module must exist in the scanner; otherwise MODULE_NOT_FOUND.
 */
...
import { enableModule } from '../../../src/tools/modules/enable.js';
...
beforeEach(async () => {
  workspace = await mkdtemp(join(tmpdir(), 'forgekit-modules-enable-'));
...
describe('enableModule — happy path', () => {
  it('creates the state file and sets enabled=true', async () => {
    await enableModule({ projectRoot: workspace, moduleId: 'forgekit_rpg' });
    const stateText = await readFile(
      join(workspace, '.forgekit', 'modules_state.json'),
      'utf8',
    );
    expect(JSON.parse(stateText)).toEqual({
      forgekit_rpg: { enabled: true },
    });
  });

  it('returns {module_id, enabled: true}', async () => {
    const result = await enableModule({
      projectRoot: workspace,
      moduleId: 'forgekit_rpg',
    });
    expect(result).toEqual({ module_id: 'forgekit_rpg', enabled: true });
  });

  it('flips a previously disabled module back to enabled via list round-trip', async () => {
    // First disable via direct state file write
    ...
    expect(before.modules[0].enabled).toBe(false);
    ...
    expect(after.modules[0].enabled).toBe(true);
  });
});

describe('enableModule — errors', () => {
  it('raises ModuleNotFoundError when the module does not exist', async () => {
    await expect(
      enableModule({ projectRoot: workspace, moduleId: 'forgekit_ghost' }),
