 ...
 *                npc / dialog / vendor tool after a valid
 *                `forgekit_rpg` license is presented. Without the
 *                license only the core-minimal set is returned.
 */
...
export interface ApplyProfileOptions {
   ...
   * supplied, the RPG-only profile unlocks all fifteen RPG subsystem
 ...
 * 'forgekit_rpg'` maps to the fifteen RPG subsystem modules (combat,
 * crafting, inventory, stats, effects, magic, equipment, progression,
 * enemies, loot, spawner, chests, npc, dialog, vendor), any other
 * non-null id raises `RpgLicenseRequiredError` for the RPG-only
 * profile.
 */
