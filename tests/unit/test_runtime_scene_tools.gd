extends GutTest
## Unit tests for McpRuntimeSceneControlTools: JSON-RPC handler adapter
## that exposes the runtime-channel scene-control MCP tools on top of a
...
# ---------------------------------------------------------------------------
# 1) runtime.get_scene_tree — forwards max_depth verbatim
# ---------------------------------------------------------------------------
...
func test_get_scene_tree_forwards_max_depth() -> void:
	...
	assert_eq(int(call.get("max_depth", -99)), 2, "max_depth forwarded")
...
func test_get_scene_tree_defaults_max_depth_to_minus_one_when_absent() -> void:
	...
	var call: Dictionary = backend.find_calls("get_scene_tree")[0]
	assert_eq(int(call.get("max_depth", -99)), -1, "max_depth defaults to -1")


# ---------------------------------------------------------------------------
# 2) runtime.get_current_scene — calls backend and returns scene info
# ---------------------------------------------------------------------------

func test_get_current_scene_calls_backend_and_returns_scene_info() -> void:
	var env: Dictionary = _new_env()
	var tools: Object = env["tools"]
	var backend: FakeRuntimeSceneControlBackend = env["backend"]
...
func test_change_scene_forwards_scene_path() -> void:
	...
	var _result: Variant = tools.change_scene({"scene_path": "res://scenes/level_2.tscn"})

	var call: Dictionary = backend.find_calls("change_scene")[0]
	assert_eq(String(call.get("scene_path", "")), "res://scenes/level_2.tscn", "scene_path forwarded")


func test_change_scene_accepts_positional_param() -> void:
	var env: Dictionary = _new_env()
	var tools: Object = env["tools"]
	var backend: FakeRuntimeSceneControlBackend = env["backend"]

	var _result: Variant = tools.change_scene(["res://scenes/boss.tscn"])

	var call: Dictionary = backend.find_calls("change_scene")[0]
	assert_eq(String(call.get("scene_path", "")), "res://scenes/boss.tscn", "positional scene_path forwarded")


# ---------------------------------------------------------------------------
# 4) runtime.reload_current_scene — calls backend and returns reload status
# ---------------------------------------------------------------------------

func test_reload_current_scene_calls_backend_and_returns_status() -> void:
	var env: Dictionary = _new_env()
	var tools: Object = env["tools"]
	var backend: FakeRuntimeSceneControlBackend = env["backend"]
	...
	var dict: Dictionary = result as Dictionary
	assert_true(dict.get("reloaded", false), "Result must contain 'reloaded: true'")


# ---------------------------------------------------------------------------
# 5) register_on — wires all four runtime scene-control methods
# ---------------------------------------------------------------------------
...
func test_register_on_wires_all_four_scene_control_methods_on_dispatcher() -> void:
	...
	var cases: Array = [
		...
		{"method": "runtime.change_scene", "params": {"scene_path": "res://x.tscn"}},
